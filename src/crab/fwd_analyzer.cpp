...
class member_component_visitor final {
  ...
  public:
    ...
    void operator()(const wto_vertex_t& c) {
        if (!_found) {
            _found = (c.node() == _node);
        }
    }

    void operator()(const wto_cycle_t& c) {
        if (!_found) {
            _found = (c.head() == _node);
            if (!_found) {
                for (const auto& x : c) {
...
{
    for (const wto_component_t& c : _wto) {
        std::visit(*this, c);
    }
    return std::make_pair(_pre, _post);
}

void interleaved_fwd_fixpoint_iterator_t::operator()(const wto_vertex_t& vertex) {
    const label_t node = vertex.node();
...
void interleaved_fwd_fixpoint_iterator_t::operator()(const wto_cycle_t& cycle) {
