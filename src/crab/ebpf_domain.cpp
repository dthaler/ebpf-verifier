...
void ebpf_domain_t::join_inv(NumAbsDomain& dst, NumAbsDomain& src) {
    ...
    if (!dst.is_bottom()) {
        for (variable_t v : variable_t::get_type_variables()) {
            add_extra_invariant(dst, extra_invariants, v, T_NUM, data_kind_t::values, src);
...
void ebpf_domain_t::forget_packet_pointers() {
    ...
    // Havoc and recompute any r*.numeric_size for packets.
    for (int i = R0_RETURN_VALUE; i <= R10_STACK_POINTER; i++) {
        auto reg_packed = reg_pack(i);

        if (variable_has_type(m_inv, reg_packed.type, T_PACKET)) {
            Reg reg{(uint8_t)i};
            havoc(reg_packed.type);
            havoc(reg_packed.packet_offset);
            havoc(reg_packed.value);
            havoc(reg_packed.numeric_size);
            recompute_numeric_size(m_inv, reg);
        }
    }
...
void ebpf_domain_t::do_store_stack(NumAbsDomain& inv, int width, const A& addr, X val_type, Y val_value,
    ...
    if (width == 8) {
        ...
        if (opt_val_ctx_offset && type_inv.has_type(m_inv, val_type, T_CTX)) {
            inv.assign(stack.store(inv, data_kind_t::ctx_offsets, addr, width, *opt_val_ctx_offset), *opt_val_ctx_offset);
        } else {
        ...
        if (opt_val_packet_offset && type_inv.has_type(m_inv, val_type, T_PACKET)) {
            inv.assign(stack.store(inv, data_kind_t::packet_offsets, addr, width, *opt_val_packet_offset), *opt_val_packet_offset);
        } else {
            stack.havoc(inv, data_kind_t::packet_offsets, addr, width);
        }

        if (opt_val_shared_offset && type_inv.has_type(m_inv, val_type, T_SHARED)) {
            inv.assign(stack.store(inv, data_kind_t::shared_offsets, addr, width, *opt_val_shared_offset), *opt_val_shared_offset);
            inv.assign(stack.store(inv, data_kind_t::shared_region_sizes, addr, width, *opt_val_shared_region_size),
    ...
    // Update any r*.numeric_size for stack type registers.
    auto updated_lb = m_inv.eval_interval(addr).lb();
    auto updated_ub = m_inv.eval_interval(addr).ub() + width;
    for (int i = R0_RETURN_VALUE; i <= R10_STACK_POINTER; i++) {
        auto reg_packed = reg_pack(i);

        if (variable_has_type(m_inv, reg_packed.type, T_STACK)) {
            auto reg_lb = m_inv.eval_interval(reg_packed.stack_offset).lb();
            auto reg_ub =
                (m_inv.eval_interval(reg_packed.stack_offset) + m_inv.eval_interval(reg_packed.numeric_size)).ub();

            // See if the register's numeric interval overlaps with changed bytes.
...
void ebpf_domain_t::recompute_numeric_size(NumAbsDomain& inv, const Reg& reg) {
    auto mem_reg = reg_pack(reg);

    if (!inv.eval_interval(mem_reg.numeric_size).is_top())
        return;

    int type = type_inv.get_type(inv, mem_reg.type);
    switch (type) {
    case T_PACKET:
        apply(inv, crab::arith_binop_t::SUB, mem_reg.numeric_size, variable_t::packet_size(), mem_reg.packet_offset,
              false);
        break;
    case T_SHARED:
        apply(inv, crab::arith_binop_t::SUB, mem_reg.numeric_size, mem_reg.shared_region_size, mem_reg.shared_offset,
              false);
        break;
    case T_STACK:
        inv.assign(mem_reg.numeric_size, stack.min_all_num_size(inv, mem_reg.stack_offset));
        break;
    }
}
                ...
                m_inv = type_inv.join_over_types(m_inv, bin.dst, [&](NumAbsDomain& inv, type_encoding_t dst_type) {
                    inv = type_inv.join_over_types(m_inv, src_reg, [&](NumAbsDomain& inv, type_encoding_t src_type) {
                        if (dst_type == T_NUM && src_type != T_NUM) {
                            ...
                            apply(inv, crab::arith_binop_t::ADD, dst_offset.value(),
                                  dst.value, get_type_offset_variable(src_reg, src_type).value(), false);
