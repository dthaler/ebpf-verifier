...
class linear_expression_t final {
  ...
  public:
    ...
    [[nodiscard]] linear_expression_t operator*(const number_t& constant) const {
        ...
        for (auto [variable, coefficient] : _variable_terms) {
    ...
    [[nodiscard]] linear_expression_t operator+(const linear_expression_t& expression) const {
        variable_terms_t variable_terms = _variable_terms;
        for (auto term : expression.variable_terms()) {
            variable_t variable = term.first;
            variable_terms[variable] = coefficient_of(variable) + term.second;
        }
    ...
    void output_variable_terms(std::ostream& o) const {
        for (auto [variable, coefficient] : variable_terms()) {
